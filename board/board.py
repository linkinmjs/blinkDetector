...
def show_board(characters, rows, columns, current_row, current_column, show_column_rect):
    ...
    board = np.full((400, 600, 3),(83, 70, 38), np.uint8)  # Crea una imagen negra
    ...
    for i, char in enumerate(characters):
        y = i // columns * paso_y
        x = i % columns * paso_x
        cv2.putText(board, char, (x + 10, y + paso_y - 10), font, 1, (106, 196, 233), 2, cv2.LINE_AA)

    # Dibuja un rectángulo alrededor de la fila seleccionada
    y_start = current_row * paso_y
    cv2.rectangle(board, (0, y_start), (board.shape[1], y_start + paso_y), (111, 81, 231), 2)

    if show_column_rect:
        # Si se está seleccionando una columna, dibuja un rectángulo alrededor de la columna actual
        x_start = current_column * paso_x
        cv2.rectangle(board, (x_start, 0), (x_start + paso_x, board.shape[0]), (111, 81, 231), 2)  # Usar otro color para diferenciar
